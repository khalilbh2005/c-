...
/// surface : retourne la surface de l'appartement
...
/// retourne la plus grande Piece de l'appartement
Piece Appartement::plusGrandePiece() const
{
    if (_lesPieces.size() ==0 )throw(invalid_argument("appartement vide"));
    int pgp = 0 ;
    for (int i=1; i<_lesPieces.size(); i++)
        if (_lesPieces[i].surface() > _lesPieces.at(pgp).surface())
            pgp = i ;
    return _lesPieces[pgp] ;
}
...
int Appartement::recherche(const string &val)const
{
    int deb,fin,millieu,ind;
    bool trouve;
    deb=0;
    fin=_lesPieces.size()-1;
    ind=-1;
    trouve=false;
    while((!trouve) && (deb<=fin))
    {
        millieu=(deb+fin)/2;
        trouve=(_lesPieces[millieu].nom()==val);
        if(trouve)
        {
            ind=millieu;
        }
...
int Appartement::recherchePremier(const string & val)const
{
    int deb,fin,millieu,ind;
    bool trouve,prem_touv;
    deb=0;
    fin=_lesPieces.size()-1;
    ind=-1;
    trouve=false;

    while((!trouve || prem_touv) && deb<=fin)
    {
        millieu=(deb+fin)/2;
        trouve=(_lesPieces[millieu].nom()==val);
        prem_touv=(_lesPieces[millieu-1].nom()==val);
        if(trouve)
        {
            if(prem_touv)
            {
    ...
    /// votre code ici, avec une valeur retourn�e diff�rente bien sur
    return (ind);
}
